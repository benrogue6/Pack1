...
class lane_track: # main class
  ...
  def image_callback(self, msg):
    ...
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV) # converting the rgb color to  hsv color format

    # inserting the range for the white color in hsv format
    ...
    if M2['m00'] > 0:#blue-cube-mask # code to dodge the blue colored cube obstacle
    ...
    cv2.imshow("white_lane_mask",mask1)
    cv2.imshow("blue_cube_mask",mask2)
    cv2.imshow("camera_output", image)
    cv2.waitKey(3)

if __name__ == '__main__':
    rospy.init_node('white_lane_track') #initialize node
